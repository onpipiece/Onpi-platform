...
app.post('/api/register', async (req, res) => {
  ...
  if (supabase) {
    const insert = { ...user, purchased_packages: JSON.stringify(user.purchased_packages) };
...
app.post('/api/forgot-password', async (req, res) => {
  ...
  // If not found by cont, try by email
  if (!user) {
    if (supabase) {
      try {
...
app.get('/api/profile', async (req, res) => {
  ...
  const user = await findUserByToken(token);
  if (!user) return res.status(401).json({ ok: false, err: 'unauthorized' });
  const { id, cont, nume, email, telegram, telefon, createdAt, purchased_packages, active_package } = user;
  const purchased = typeof purchased_packages === 'string' ? JSON.parse(purchased_packages || '[]') : (purchased_packages || []);
  res.json({ ok: true, user: { id, cont, nume, email, telegram, telefon, createdAt, purchased_packages: purchased, active_package } });
});
...
app.get('/api/user/:cont', async (req, res) => {
  ...
  const { id, cont: c, nume, email, telegram, telefon, createdAt, purchased_packages, active_package, wallet, balance, stake } = user;
  const purchased = typeof purchased_packages === 'string' ? JSON.parse(purchased_packages || '[]') : (purchased_packages || []);
  res.json({ ok: true, user: { id, cont: c, nume, email, telegram, telefon, createdAt, purchased_packages: purchased, active_package, wallet, balance, stake } });
});
